...
export function ProjectModule({ notes, onOpen }: Props) {
    ...
    <section>
      ...
      {open && (
        <div className="space-y-0.5 mt-1">
          {notes.length === 0 ? (
            <p className="px-2 text-[11px] text-slate-600 italic">No project notes yet.</p>
          ) : (
            notes.map((note, i) => (
              <div
                key={i}
              ...
              >
                <span className="material-symbols-outlined text-lg text-slate-500 group-hover:text-primary shrink-0">folder_open</span>
